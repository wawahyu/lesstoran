...
<div class="container">
                    <div class="row margint90">
                        <ul class="breadcrumb">
                            <li><a href="#">Home</a></li>
                            <li><a href="{{url('admin/order/')}}">Menu</a></li>
                            ...
                            <div>
                                ...
                                <ul class="nav nav-tabs">
                                    ...
                                    <li @if (!@$filter) class="active" @endif>
                                        <a href="{{url('admin/order')}}"><span class="fa fa-list"></span> List</a>           
                                    </li>
                                    <li @if (@$filter == 'makanan') class="active" @endif>
                                        <a href="{{url('admin/order/filter/makanan')}}"><span class="glyphicon glyphicon-cutlery"></span> Makanan</a>            
                                    </li> 
                                    <li @if (@$filter == 'minuman') class="active" @endif>
                                        <a href="{{url('admin/order/filter/minuman')}}"><span class="glyphicon glyphicon-glass"></span> Minuman</a>           
                                    </li>            
                                </ul>
...
<script id="details-layouts" type="text/x-handlebars-layouts">
    <form id="form-order" action="{{ url('admin/order') }}/@{{id}}/add" class="form-horizontal" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    <table class="table">
        <tr>
            <td>Gambar</td>
...
$(document).ready(function(){
    ...
    var table = $('#order-table').DataTable({
        ...
        serverSide: true,
        @if(!@$filter)
        ajax: "{{ url('admin/order/datatables')}}",
        @else
        ajax: "{{ url('admin/order/datatables/filter') }}/{{$filter}}",
        @endif
        columns: [
