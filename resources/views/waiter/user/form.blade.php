...
<div class="container">
                    <div class="row margint90">
                        <ul class="breadcrumb">
                            ...
                            <li><a href="{{url('waiter/user/filter')}}/{{$filter}}">{{ucfirst($filter)}}</a></li>
            ...
            <div class="box-header with-border">
                <a href="{{ url('waiter/user/filter')}}/{{@$filter}}" class="btn bg-purple"><i class="fa fa-chevron-left"></i> Kembali</a>
            </div>
            <div class="box-body">
                <form id="form-user" action="{{ empty($data) ? url('waiter/user/store') : url("waiter/user/$data->id/update") }}" class="form-horizontal" method="POST" enctype="multipart/form-data">
                {{ csrf_field() }}
                @if (!empty($result))
                    {{ method_field('patch') }}
                @endif
