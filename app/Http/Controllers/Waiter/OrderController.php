...
class OrderController extends Controller
{
    ...
    public function order_index(){
        $title = 'Menu';
        return view('waiter.order.index', compact('title'));
    }
    ...
    public function order_filter($filter){
        if ($filter == 'makanan' || $filter == 'minuman') {
            ...
            return view('waiter.order.index', compact('title', 'filter'));
        }
        else{
    ...
    public function order_myorder_index(){
        $title = 'Pesanan';
        return view('waiter.order.myindex', compact('title'));
    }
    ...
    public function order_myorder_filter($filter){
        if ($filter == 'makanan' || $filter == 'minuman') {
            # code...
            $title = 'Daftar '.ucfirst($filter);
            return view('waiter.order.myindex', compact('title', 'filter'));
        }
        else{
            return redirect('waiter/order');
        }
    ...
    public function order_add(Request $request, $id){
        ...
        if ($status) {
            return redirect('waiter/order')->with('success','Pesanan Berhasil <a href="#">Ditambahkan</a>');
        }else
        {
    ...
    public function order_edit(Request $request, $id){
        ...
        $status = $detailOrder->save();
        if ($status) {
            return redirect('waiter/order/myorder')->with('success','Pesanan Berhasil <a href="#">Diubah</a>');
        }else
        {
    ...
    public function order_send(Request $request){
        ...
        if ($status) {
            if ($status) {
                ...
                return redirect('waiter/order')->with('success','<i class="fa fa-check"></i> Pesanan Berhasil <a href="#">Dikirim</a>');
            }
        }else
        {
            return redirect()->back()->with('error', 'Pesanan Gagal Ditambahkan');
