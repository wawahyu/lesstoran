...
class OrderController extends Controller
{
    ...
    public function order_index(){
        $title = 'Menu';
        return view('customer.order.index', compact('title'));
    }
    ...
    public function order_filter($filter){
        if ($filter == 'makanan' || $filter == 'minuman') {
            ...
            return view('customer.order.index', compact('title', 'filter'));
        }
        else{
    ...
    public function order_myorder_index(){
        $title = 'Pesanan';
        return view('customer.order.myindex', compact('title'));
    }
    ...
    public function order_myorder_filter($filter){
        if ($filter == 'makanan' || $filter == 'minuman') {
            # code...
            $title = 'Daftar '.ucfirst($filter);
            return view('customer.order.myindex', compact('title', 'filter'));
        }
        else{
            return redirect('/order');
        }
    ...
    public function order_send(Request $request){
        ...
        if ($status) {
            if ($status) {
                ...
                return redirect('order')->with('success','<i class="fa fa-check"></i> Pesanan Berhasil <a href="#">Dikirim</a>');
            }
        }else
        {
            return redirect()->back()->with('error', 'Pesanan Gagal Ditambahkan');
